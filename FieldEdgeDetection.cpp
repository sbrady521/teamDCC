...
int * chromaRange = getChromaRange(VisionFrame &frame);
int * getRGB(int x, int y, VisionFrame &frame, bool top);
...
//      }
   ...
   int * chromaRange = getChromaRange(frame);
   ...
   for (i = 0; i < cols; ++i) {
      ...
      for (; j < fovea_end; ++ j, ++ c) {
          ...
          int * rgb = getRGB(i, j, frame, true);
          int total = rgb[0] + rgb[1] + rgb[2];
          float chroma = rgb[1]/total;
          free(rgb);
          bool green = false;
          if (chroma <= chromaRange[1] && chroma >= chromaRange[0]){
              green = true;
          }
...
int * getRGB(int x, int y, VisionFrame &frame, bool top){
    ...
    int col_len;
    if(top == true){
        image = frame->topImage;
        col_len = 1280;
    }else{
        image = frame->botImage;
        col_len = 640;
    }

    int y = gety(image, x, y, col_len);
    int u = getu(image, x, y, col_len);
    int v = getv(image, x, y, col_len);
...
int * yuv_to_rgb(int y, int u, int v){
    ...
    rgb[0] = y + 1.4075 * (v - 128);
    rgb[1] = y - 0.3455*(u - 128) - 0.7169 * (v - 128);
    rgb[2] = y + 1.779*(u - 128);
    return rgb;
